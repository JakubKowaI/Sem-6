...
println("\nZADANIE c (Aktualizacja wykresów):")
order_z = LexOrder([3, 2, 1]) # z > y > x
stozek = x*x + y*y - z*z      # Stożek: x^2 + y^2 - z^2 = 0

f_warianty = [
    monomial(c + 1.0, [0, 0, 1], 3),               # c.1: (c+1)z = 10z
    z + C(d + 1.0),                                 # c.2: z + 8 = 0
    x + z - C(e_val + 1.0),                         # c.3: x + z - 1 = 0
    x + y + z + C(f_val + 1.0),                     # c.4: x + y + z + 7 = 0
    monomial(1.0/a, [0, 1, 0], 3) + z + C(1.0)      # c.5: 0.5y + z + 1 = 0
]

tytuly_c = [
    "Wariant c.1 (Punkt osobliwy (0,0))", 
    "Wariant c.2 (Okrąg)", 
    "Wariant c.3 (Parabola)", 
    "Wariant c.4 (Hiperbola ukośna)", 
    "Wariant c.5 (Elipsa)"
]
...
for i in 1:5
    ...
    if i == 1
        # Wariant c.1 daje pojedynczy punkt (0,0). Contour go nie wyrysowuje, wymuszamy punkt:
        contour(xs_c, ys_c, wyszukane_wartosci, levels=[0.0], color=:blue, lw=2,
                title=tytuly_c[i], xlabel="x", ylabel="y", aspect_ratio=:equal, legend=false)
        scatter!([0.0], [0.0], color=:blue, markersize=6, label="Rozmaitosc (0,0)")
    else
        contour(xs_c, ys_c, wyszukane_wartosci, levels=[0.0], color=:blue, lw=2,
                title=tytuly_c[i], xlabel="x", ylabel="y", aspect_ratio=:equal, legend=false)
    end
    
    filename = "./L3/plots/wykres_c$(i).png"
    savefig(filename)
...
ys_d = range(-10.0, 10.0, length=200)
zs_d = range(-10.0, 10.0, length=200)
macierz_d1 = [ewaluuj(poly_d1, [0.0, yv, zv]) for zv in zs_d, yv in ys_d]

contour(ys_d, zs_d, macierz_d1, levels=[0.0], color=:red, lw=2,
        title="Zadanie d: Rzut ukladu na plaszczyzne YZ", xlabel="y", ylabel="z", aspect_ratio=:equal, legend=false)
savefig("./L3/plots/wykres_d1.png")
...
contour(xs_d2, ys_d2, f_d2, levels=[0.0], color=:darkgreen, lw=2,
        title="Zadanie d: Przekroj dla z=2 (Kardioida)", xlabel="x", ylabel="y", aspect_ratio=:equal, legend=false)
savefig("./L3/plots/wykres_d2.png")
...
ys_e = range(-70.0, 70.0, length=300)
macierz_e = [ewaluuj(poly_e, [xv, yv, 0.0]) for yv in ys_e, xv in xs_e]

contour(xs_e, ys_e, macierz_e, levels=[0.0], color=:purple, lw=2,
        title="Zadanie e: Trysektrysa Maclaurina", xlabel="x", ylabel="y", aspect_ratio=:equal, legend=false)
savefig("./L3/plots/wykres_e.png")
